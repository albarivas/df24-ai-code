...
import sendEmail from '@salesforce/apex/AttendeePassController.sendEmail';

export default class DisplayAttendeePass extends LightningElement {

    @api attendeePassBody;
    firstRender = true;
    ...
    renderedCallback() {
        ...
        console.log(this.attendeePassBody);
        this.attendeePassBody = this.attendeePassBody.replace('```html', '');
        this.attendeePassBody = this.attendeePassBody.replace('```', '');
        
    }

    handleClick() {
        sendEmail({emaiLBody: this.attendeePassBody})
        .then(result => {
